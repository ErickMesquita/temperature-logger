...
def request_api(api_host: str, api_endpoint: str) -> float:
    try:
        dados = float(requests.get(f"http://{api_host}/{api_endpoint}", timeout=5).text)
    except requests.exceptions.ConnectTimeout:
        dados = None
    return dados

def get_values(api_host: str):
    temperature = request_api(api_host, "temperature")
    humidity    = request_api(api_host, "humidity")
